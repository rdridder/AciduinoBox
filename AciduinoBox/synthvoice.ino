...
//  Filter.SetMode(TeeBeeFilter::LP_18);
...
#endif
  highpass1.setSampleRate(SAMPLE_RATE * OVERSAMPLE_303);
  allpass.setSampleRate(SAMPLE_RATE * OVERSAMPLE_303); 
  ...
  antiAliasFilter.setSampleRate(OVERSAMPLE_303 * SAMPLE_RATE);
  antiAliasFilter.setMode(BiquadFilter::LOWPASS12);
  antiAliasFilter.setGain(0.0f);
  antiAliasFilter.setFrequency( SAMPLE_RATE * 0.5f);
}
...
inline float SynthVoice::getSample() {
    ...
    for (int j = 1; j<=OVERSAMPLE_303; j++) {
      if (AmpEnv.isRunning()) {
      ...
      _phaze += _currentStep;
      #if OVERSAMPLE_303 > 1
      if ( _phaze >= TABLE_SIZE) _phaze -= TABLE_SIZE ; // requires oversampling
      #elif OVERSAMPLE_303 == 1
      ...
      #if OVERSAMPLE_303 > 1
      samp = antiAliasFilter.getSample(samp);
      #endif
    }
